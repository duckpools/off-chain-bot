...
from consts import INTEREST_MULTIPLIER, MIN_BOX_VALUE, INTEREST_FREQUENCY_POLL, MAX_BORROW_TOKENS, TX_FEE, ERROR, \
    DOUBLE_SPENDING_ATTEMPT, BorrowTokenDenomination, MAX_INTEREST_SIZE, MAX_CHILD_EXECUTION_FEE, MAX_TX_FEE
...
def create_new_child(pool, head_child):
...
def t_update_interest_rate(pool, curr_height, latest_tx, dummy_script, fee=randbelow(1300001 - 1100000) + 1100000):
    if pool["version"] == 2:
        return t_update_interest_rate_v2(pool, curr_height, latest_tx, dummy_script, fee)
...
def t_update_interest_rate_v2(pool, curr_height, latest_tx, dummy_script, fee=randbelow(1300001 - 1100000) + 1100000):
    dummy_box = get_dummy_box(dummy_script)
    logger.info("Starting Interest Rate Job")
