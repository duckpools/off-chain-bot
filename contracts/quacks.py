...
def generate_pool_script(collateralContractScript, childBoxNft, parameterBoxNft):
	...
	val MaxLendTokens = 9000000000000010L // Genesis value must give lendTokenValue = 1xLendTokenMultiplier and initialCircSupply non zero
	...
	val LiquidationFeesMinimum = 4000000L
	...
	val isValidBorrow = if (plausibleCollateralBoxes.size > 0) {{
		...
		val loanSettingsRecorded = collateralBox.R9[Coll[Long]].get
		...
		val isAboveMinimumValue = collateralValue > LiquidationFeesMinimum && collateralValue >= minimumValue
		val isValidSettings = (
			loanSettingsRecorded(0) == threshold &&
		...
		val isAssetAmountValid = deltaAssetsInPool * -1 == loanAmount && loanAmount >= minimumLoanAmount
		val isTotalBorrowedValid = deltaTotalBorrowed == collateralBorrowTokens._2
		val isCollateralBorrowTokenIdValid = collateralBorrowTokens._1 == currentBorrowTokens._1
		(
			commonConditions &&
			...
			isCollateralBorrowTokenIdValid &&
...
def generate_collateral_script(repaymentScript, interestNft, poolCurrencyId):
	...
	if (fQuotes.size > 0) {{
		...
		val iPenaltyQuoted = quoteReport(3)
		...
		if (fCollaterals.size > 0) {{
			...
			val fLoanSettings = fCollateral.R9[Coll[Long]].get
			val fBufferGap = fLoanSettings(2)
			...
			if (fRepayments.size > 0) {{
				...
				val readyToLiquidate = sigmaProp(
					...
					fBufferLiquidation > HEIGHT + iBufferGap && fBufferLiquidation < HEIGHT + iBufferGap + 5 &&
					!isSufficientCollateral &&
					retainLoanSettings &&
					isOnlyOneCollateralInput			
				)
				...
				val resetLiquidate = sigmaProp(
					...
					fCollateralValue <= currentValue + 10 * MinimumTransactionFee &&
					fCollateral.tokens == SELF.tokens &&
					iBufferLiquidation != defaultBuffer &&
				...
				sigmaProp(
					...
					isSufficientCollateral &&
					isValidNewSettings &&
					collateralInputs.size == fCollaterals.size &&
...
def generate_logic_script_simple():
    return compile_script(f'''{{	
	val Slippage = 2.toBigInt
	val SlippageDenom = 100.toBigInt
	val DexFeeDenom = 1000.toBigInt
...
def generate_logic_script_lp():
	...
	val lpBorrowCirc = (LP_MaxBorrowTokens - lpPoolBox.tokens(2)._2).toBigInt
	...
	val allAssetsCounted = boxToQuote.tokens.slice(1, boxToQuote.tokens.size).forall{{
		(token: (Coll[Byte], Long)) => zippedOrderedAssetsList.exists {{
			(reportedToken: (Coll[Byte], Long)) =>
				if (token._1 == LP_TOKEN_ID) {{
					reportedToken._1 == token._1 &&
					reportedToken._2.toBigInt == token._2.toBigInt * lpConversionNum / lpConversionDenom
				}} else {{
					reportedToken == token
				}}
		}}
